...
const deleteBlog=({success, fail, data})=>{
    Blog.delete(BLOGURL.deleteBlog+`?blogid=${data.blogId}`)
        .then((res) => {
            if (res.status === 200) {
                success(res);
            }
...
export default { fetchBlogs, fetchBlog, fetchBlogByTag, fetchBlogByUserId, createBlog ,deleteBlog}